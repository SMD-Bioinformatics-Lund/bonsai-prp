...
PRP no longer has any result-parsing subcommands (`parse jasen`, `format-cdm`, `validate-result`); only `bonsai upload` and `bonsai bootstrap` remain. The per-software JASEN result parsers now live in [bonsai-libs](https://github.com/mhkc/bonsai-libs) (`bonsai_libs.parse`), and CDM input generation now lives in [jasentool](https://github.com/SMD-Bioinformatics-Lund/jasentool) (`jasentool format-cdm`).
