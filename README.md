...
PRP no longer has any result-parsing subcommands (`parse jasen`, `format-cdm`, `validate-result`); only `bonsai upload` and `bonsai bootstrap` remain. The JASEN result-parsing library and pipeline/manifest orchestration now live in [bonsai-libs](https://github.com/mhkc/bonsai-libs) (`bonsai_libs.parse`, `bonsai_libs.pipeline`), and CDM input generation now lives in [jasentool](https://github.com/SMD-Bioinformatics-Lund/jasentool) (`jasentool format-cdm`).
...
Bootstrap a new test instance of Bonsai by setting up users and groups from a configuration file.
