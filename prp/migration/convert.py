...
def migrate_result(old_result: UnformattedResult) -> PipelineResult:
    ...
    if input_schema_version not in ALL_FUNCS:
        ...
        raise ValueError(f"Unknown result version, found {input_schema_version} expected any of {all_versions}")
...
def v1_to_v2(result: UnformattedResult) -> UnformattedResult:
    ...
    upd_profile: list[str] = [prof.strip() for prof in result['pipeline']['analysis_profile'].split(',')]
