...
from prp.models.hamronization import HamronizationEntry, InputSequence, ReferenceSequence, BaseSequenceRecord
...
def parse_hamronization(source: TextIO, null_values: list[str] = ['NA', '-']) -> HamronizationEntries:
    """Parse harmonization format."""
    delimiter = '\t'
    header = [col.strip().lower().replace(' ', '_') for col in source.readline().split(delimiter)]
    ...
    for row in creader:
        ...
        clean_row = {k: normalize_cell(v, percent_mode='percent') for k, v in row.items() if k != "strand_orientation"}
        ...
        accnr = clean_row.get('reference_accession') if clean_row.get('reference_accession') else "unknown"
        ...
        # convert strand
        strand_orientation = _convert_strand_orientation(clean_row.get('strand_orientation'))
